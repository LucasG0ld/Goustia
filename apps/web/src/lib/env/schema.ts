...
export const serverEnvSchema = publicEnvSchema.extend({
  ...
  AI_TEXT_PROVIDER: z.string().trim().min(1).default("groq"),
  AI_TEXT_MODEL: z.string().trim().min(1).default("openai/gpt-oss-120b"),
  GROQ_API_KEY: optionalSecret,
  AI_TEXT_FALLBACK_PROVIDER: z.string().trim().min(1).default("cloudflare"),
  CLOUDFLARE_TEXT_MODEL: z
    .string()
    .trim()
    .min(1)
    .default("@cf/qwen/qwen3-30b-a3b-fp8"),
  AI_IMAGE_PROVIDER: z.string().trim().min(1).default("cloudflare"),
  AI_IMAGE_MODEL: z
    .string()
    .trim()
    .min(1)
